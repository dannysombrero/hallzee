...
Chrome version is selected. Managed Chromebook and physical Windows/Mac BLE
...
Changing hostname, scheme, port or Chrome profile creates a different storage
and permission boundary. Plan owner release/reclaim and data backup explicitly;
restoring data does not restore ownership. Publish exact matching AGPL source
...
The bundle targets Chrome 116 syntax, but feature detection governs operation;
...
service workers/cache storage for offline use. Document PiP is optional. Saved
...
Chrome policies such as `DefaultWebBluetoothGuardSetting`,
`WebBluetoothAskForUrls` and `WebBluetoothBlockedForUrls` for the deployed Chrome
...
A Mac covers shared automated checks only. Windows requires a Windows BLE PC for
OS passkey, encrypted GATT, bond reuse, reconnect/sleep and installed-PWA checks;
**Windows behavior is unverified**. Managed ChromeOS acceptance remains separate.
