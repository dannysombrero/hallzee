...
Start the receiver while `Hallzee` is powered and no Android serial
...
```sh
./bathroom-receiver "Hallzee" "/Users/danny/Documents/hallzee_trips.csv"
```
...
is `%LOCALAPPDATA%\\Hallzee\\hallzee-trips.db`; CSV files are sorted
...
`Hallzee`, and choose **Sync Now**. The app handles pairing (PIN
...
`%LOCALAPPDATA%\\Hallzee\\startup-errors.log`.
...
new timestamped export under `Documents/Hallzee/exports`; use
